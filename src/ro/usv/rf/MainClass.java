...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		FileUtils.writeLearningSetToFile("scaledSet.csv",
		autoscaleLearningSet(learningSet, featureAverages));

	}

	private static double[][] autoscaleLearningSet(double[][] learningSet, double[] featureAverages) {
		
		int numOfForms = learningSet.length;
		int numOfFeatures = learningSet[0].length - 1;
		
		double[][] autoscaledLearningSet = new double[numOfForms][numOfFeatures];
		
		if(featureAverages != null && featureAverages.length > 0) {
			for(int i = 0; i < numOfForms; ++i) {
				for(int j = 0; j < numOfFeatures; ++j) {			
					double firstDiff = learningSet[i][j] - featureAverages[i];
					
					double secondDiff = 0;
					
					for(int k = 0; k < numOfForms; ++k) {
						secondDiff += Math.pow(learningSet[k][j] - featureAverages[i], 2);
					}
					
					autoscaledLearningSet[i][j] = firstDiff / secondDiff;
				}
			}
		}
